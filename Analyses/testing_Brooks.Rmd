---
title: "PERFect filtering for mock data set 2"
...
1. *Mock community data 2: bias experiment data*.
...
```{r}
#Input mock data set 2
data(mock2)
Counts <- mock2$Counts
TrueComp <- mock2$TrueTaxa

true <- names(Counts) %in% TrueComp
#make a vector of taxa cont and  id's
type <-  rep("contam", dim(Counts)[2])
type[true] <- "true"

taxa <- names(Counts)
#add numeric id
id <- seq(1:dim(Counts)[2])
taxaInfo <- data.frame(id, taxa, type)
...
resSummary <-function(X, filtX, taxaInfo, time = NA){
  ...
  ntrue <- sum(taxaInfofilt$taxa %in% TrueComp)
  ncont <- length(taxaInfo$taxa) - 7
...
#True taxa ranking in each ordering
