...
path <- "~/Dropbox/PERFect/RCode/sourcetracker-master/"
# load sample metadata
metadata <- read.table(paste0(path,'data/metadata.txt'),sep='\t',h=T,row.names=1,check=F,comment='')
...
otus <- read.table(paste0(path,'data/otus.txt'),sep='\t', header=T,row.names=1,check=F,skip=1,comment='')
...
#results predicted by sourse-tracker for that data
results$proportions

# Estimate leave-one-out source proportions in training data 
results.train <- predict(st, alpha1=alpha1, alpha2=alpha2)
saveRDS(results, "~/Dropbox/PERFect/RCode/ST.rds")
save.image(paste0(path, "Results_NICU.RData"))
# plot results
